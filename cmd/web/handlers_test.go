...
func TestShowGist(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid ID", "/gist/1", http.StatusOK, []byte("An old silent pond...")},
		{"Non-existent ID", "/gist/2", http.StatusNotFound, nil},
		{"Negative ID", "/gist/-1", http.StatusNotFound, nil},
		{"Decimal ID", "/gist/1.23", http.StatusNotFound, nil},
		{"String ID", "/gist/foo", http.StatusNotFound, nil},
		{"Empty ID", "/gist/", http.StatusNotFound, nil},
		{"Trailing slash", "/gist/1/", http.StatusNotFound, nil},
	}
...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Duplicate email", "Bob", "dupe@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("Email Address is already in use")},
